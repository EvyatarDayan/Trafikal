//
//  StringKey.swift
//  Trafikal
//

import Foundation

/// Keys for `Data/strings.json` UI copy.
enum StringKey: String, CaseIterable {
    // Tabs
    case tabStudy = "tab.study"
    case tabCategories = "tab.categories"
    case tabGoal = "tab.goal"
    case goalSubtitle = "goal.subtitle"
    case tabHome = "tab.home"
    case tabTests = "tab.tests"
    case tabPractice = "tab.practice"
    case tabSigns = "tab.signs"
    case tabQuestions = "tab.questions"
    case tabSettings = "tab.settings"

    case practiceTitle = "practice.title"

    // Favorites
    case favoritesTitle = "favorites.title"
    case favoritesAddSign = "favorites.add_sign"
    case favoritesRemoveSign = "favorites.remove_sign"
    case favoritesAddQuestion = "favorites.add_question"
    case favoritesRemoveQuestion = "favorites.remove_question"
    case favoritesEmptySigns = "favorites.empty_signs"
    case favoritesEmptyQuestions = "favorites.empty_questions"
    case favoritesAddedSign = "favorites.added_sign"
    case favoritesAddedQuestion = "favorites.added_question"
    case favoritesDontShowAgain = "favorites.dont_show_again"

    // Welcome
    case welcomeTitlePrefix = "welcome.title_prefix"
    case welcomeTitleBrand = "welcome.title_brand"
    case welcomeSubtitle = "welcome.subtitle"
    case welcomeStart = "welcome.start"

    // Home
    case homeSignOfToday = "home.sign_of_today"
    case homeNoSignsLoaded = "home.no_signs_loaded"
    case homeStudyThisSign = "home.study_this_sign"
    case homeDetails = "home.details"
    case homeInfo = "home.info"
    case homeShuffle = "home.shuffle"
    case homeShortcutSignTest = "home.shortcut_sign_test"
    case homeShortcutQuestionTest = "home.shortcut_question_test"
    case homeShortcutSimulationTest = "home.shortcut_simulation_test"
    case homeShortcutPracticeSigns = "home.shortcut_practice_signs"
    case homeShortcutPracticeQuestions = "home.shortcut_practice_questions"
    case homeMaterialCoverageTitle = "home.material_coverage_title"
    case homeMaterialCoverageSubtitle = "home.material_coverage_subtitle"
    case homeMaterialCoverageSigns = "home.material_coverage_signs"
    case homeMaterialCoverageQuestions = "home.material_coverage_questions"
    case homeMaterialCoverageCount = "home.material_coverage_count"
    case homeExamReadinessTitle = "home.exam_readiness_title"
    case homeExamReadinessBasedOn = "home.exam_readiness_based_on"
    case homeExamReadinessNotReady = "home.exam_readiness_not_ready"
    case homeExamReadinessGettingClose = "home.exam_readiness_getting_close"
    case homeExamReadinessReady = "home.exam_readiness_ready"
    case homeExamReadinessNoTests = "home.exam_readiness_no_tests"
    case homeTestStatistics = "home.test_statistics"
    case homeNoTestsYet = "home.no_tests_yet"
    case homeTakePracticeTest = "home.take_practice_test"
    case homeLastTest = "home.last_test"
    case homeRecentAverage = "home.recent_average"
    case homeTestsTaken = "home.tests_taken"
    case homeBestScore = "home.best_score"
    case homeLastNTests = "home.last_n_tests"
    case homeAllTime = "home.all_time"
    case homePersonalBest = "home.personal_best"
    case homeKeepLearning = "home.keep_learning"
    case homeExploreDescription = "home.explore_description"
    case homeComingSoon = "home.coming_soon"
    case homeLicenseStepsTitle = "home.license_steps_title"
    case homeLicenseStepsProgress = "home.license_steps_progress"

    // Driving license steps
    case licenseStepNumber = "license.step_number"
    case licenseMarkDone = "license.mark_done"
    case licenseMarkNotDone = "license.mark_not_done"
    case licensePreviousItemsNotDone = "license.previous_items_not_done"
    case licensePermitTitle = "license.permit.title"
    case licensePermitDetail = "license.permit.detail"
    case licenseEyeCheckTitle = "license.eye_check.title"
    case licenseEyeCheckDetail = "license.eye_check.detail"
    case licenseRisk1Title = "license.risk1.title"
    case licenseRisk1Detail = "license.risk1.detail"
    case licenseRisk2Title = "license.risk2.title"
    case licenseRisk2Detail = "license.risk2.detail"
    case licenseTheoryTestTitle = "license.theory_test.title"
    case licenseTheoryTestDetail = "license.theory_test.detail"
    case licenseDrivingLessonsTitle = "license.driving_lessons.title"
    case licenseDrivingLessonsDetail = "license.driving_lessons.detail"
    case licenseDrivingTestTitle = "license.driving_test.title"
    case licenseDrivingTestDetail = "license.driving_test.detail"
    case licensePickUpTitle = "license.pick_up.title"
    case licensePickUpDetail = "license.pick_up.detail"
    case licenseAllStepsCompleted = "license.all_steps_completed"

    // Signs tab (study cards)
    case studyTitle = "study.title"
    case studyAllSigns = "study.all_signs"
    case studyCardsInOrder = "study.cards_in_order"
    case studyPractice = "study.practice"
    case studyAboutImages = "study.about_images"
    case studyQuickStart = "study.quick_start"
    case studyPrevious = "study.previous"
    case studyNext = "study.next"
    case studyOf = "study.of"
    case studyMeaning = "study.meaning"

    // Categories
    case categoriesTitle = "categories.title"
    case categoriesSignCount = "categories.sign_count"
    case categoriesSearch = "categories.search"

    // Sign categories
    case categoryWarningTitle = "category.warning.title"
    case categoryWarningSubtitle = "category.warning.subtitle"
    case categoryPriorityTitle = "category.priority.title"
    case categoryPrioritySubtitle = "category.priority.subtitle"
    case categoryProhibitionTitle = "category.prohibition.title"
    case categoryProhibitionSubtitle = "category.prohibition.subtitle"
    case categoryMandatoryTitle = "category.mandatory.title"
    case categoryMandatorySubtitle = "category.mandatory.subtitle"
    case categoryInformationTitle = "category.information.title"
    case categoryInformationSubtitle = "category.information.subtitle"
    case categoryAdditionalTitle = "category.additional.title"
    case categoryAdditionalSubtitle = "category.additional.subtitle"

    // Sign test (Signs tab)
    case signsTitle = "signs.title"
    case signsInstructions = "signs.instructions"
    case signsStartNew = "signs.start_new"
    case signsPreviousResults = "signs.previous_results"
    case signsYourLastQuiz = "signs.your_last_quiz"
    case signsStartOver = "signs.start_over"
    case signsNoSigns = "signs.no_signs"
    case signsPreparing = "signs.preparing"
    case signsWhatDoesSignMean = "signs.what_does_sign_mean"
    case signsEnd = "signs.end"
    case signsRoadSignQuiz = "signs.road_sign_quiz"

    // Tests hub
    case testsTitle = "tests.title"
    case testsTabSimulate = "tests.tab.simulate"
    case testsYourLastResults = "tests.your_last_results"
    case testsLastResultsEmpty = "tests.last_results_empty"

    // Exam simulation
    case simulationTitle = "simulation.title"
    case simulationHistoryTitle = "simulation.history_title"
    case simulationInstructions = "simulation.instructions"
    case simulationStartNew = "simulation.start_new"
    case simulationPreviousResults = "simulation.previous_results"
    case simulationYourLastQuiz = "simulation.your_last_quiz"
    case simulationUnavailable = "simulation.unavailable"
    case simulationTimeLeft = "simulation.time_left"
    case simulationTimeUp = "simulation.time_up"
    case simulationPassed = "simulation.passed"
    case simulationFailedAlmostThere = "simulation.failed_almost_there"
    case simulationFailedLow = "simulation.failed_low"
    case simulationPassLabel = "simulation.pass_label"
    case simulationFailLabel = "simulation.fail_label"
    case testNextQuestion = "test.next_question"
    case testShowResults = "test.show_results"
    case testCorrectCount = "test.correct_count"
    case testIncorrectCount = "test.incorrect_count"
    case testScoreSummary = "test.score_summary"
    case testCorrectLabel = "test.correct_label"
    case testExitAlertTitle = "test.exit_alert_title"
    case testExitAlertMessage = "test.exit_alert_message"

    // Questions
    case questionsTitle = "questions.title"
    case questionsAllQuestions = "questions.all_questions"
    case questionsCountFormat = "questions.count_format"
    case questionsBrowse = "questions.browse"
    case questionsSearch = "questions.search"
    case questionsExplanation = "questions.explanation"
    case questionsInstructions = "questions.instructions"
    case questionsStartNew = "questions.start_new"
    case questionsPreviousResults = "questions.previous_results"
    case questionsYourLastQuiz = "questions.your_last_quiz"
    case questionsTheoryQuiz = "questions.theory_quiz"
    case questionsStartOver = "questions.start_over"
    case questionsEnd = "questions.end"
    case questionsNoQuestions = "questions.no_questions"
    case questionsPreparing = "questions.preparing"

    // History
    case historyTitle = "history.title"
    case historyCompletedTests = "history.completed_tests"
    case historyFilterAll = "history.filter.all"
    case historyFilterSigns = "history.filter.signs"
    case historyFilterQuestions = "history.filter.questions"
    case historyFilterSimulation = "history.filter.simulation"
    case historyEmptyTitle = "history.empty_title"
    case historyEmptyMessage = "history.empty_message"
    case historyEmptyFilteredTitle = "history.empty_filtered.title"
    case historyEmptyFilteredMessage = "history.empty_filtered.message"
    case historyGotIt = "history.got_it"
    case historyViewMistakes = "history.view_mistakes"
    case historyMistakesTitle = "history.mistakes_title"
    case historyMistakeProgress = "history.mistake_progress"
    case historyNoMistakesTitle = "history.no_mistakes.title"
    case historyNoMistakesMessage = "history.no_mistakes.message"
    case historyPercentCorrect = "history.percent_correct"
    case historyDetailFormat = "history.detail_format"

    // Settings
    case settingsTitle = "settings.title"
    case settingsPreferences = "settings.preferences"
    case settingsLanguage = "settings.language"
    case settingsLanguageSubtitle = "settings.language_subtitle"
    case settingsDarkMode = "settings.dark_mode"
    case settingsDarkModeSubtitle = "settings.dark_mode_subtitle"
    case settingsNotificationsSection = "settings.notifications_section"
    case settingsNotificationsScheduleTitle = "settings.notifications_schedule_title"
    case settingsNotificationsScheduleSubtitle = "settings.notifications_schedule_subtitle"
    case settingsNotificationsOff = "settings.notifications_off"
    case settingsNotificationsNavTitle = "settings.notifications_nav_title"
    case settingsNotificationsDailyReminder = "settings.notifications_daily_reminder"
    case settingsNotificationsDailyReminderSubtitle = "settings.notifications_daily_reminder_subtitle"
    case settingsNotificationsFooter = "settings.notifications_footer"
    case settingsNotificationsAlertTitle = "settings.notifications_alert_title"
    case settingsNotificationsPermissionDenied = "settings.notifications_permission_denied"
    case settingsNotificationsContentTitle = "settings.notifications_content_title"
    case settingsNotificationsContentBody = "settings.notifications_content_body"
    case settingsTestHistory = "settings.test_history"
    case settingsClearHistory = "settings.clear_history"
    case settingsClearHistorySubtitle = "settings.clear_history_subtitle"
    case settingsClearAlertTitle = "settings.clear_alert_title"
    case settingsClearAlertMessage = "settings.clear_alert_message"
    case settingsCancel = "settings.cancel"
    case settingsClear = "settings.clear"
    case settingsExportSection = "settings.export_section"
    case settingsExportFavoritesTitle = "settings.export_favorites_title"
    case settingsExportFavoritesSubtitle = "settings.export_favorites_subtitle"
    case settingsExportFavoritesAlertTitle = "settings.export_favorites_alert_title"
    case settingsExportFavoritesEmpty = "settings.export_favorites_empty"
    case settingsExportFavoritesFailed = "settings.export_favorites_failed"
    case settingsExportFavoritesPDFTitle = "settings.export_favorites_pdf_title"
    case settingsExportFavoritesPDFSummary = "settings.export_favorites_pdf_summary"
    case settingsExportFavoritesPDFCode = "settings.export_favorites_pdf_code"
    case settingsExportFavoritesPDFName = "settings.export_favorites_pdf_name"
    case settingsExportFavoritesPDFCategory = "settings.export_favorites_pdf_category"
    case settingsExportFavoritesPDFMeaning = "settings.export_favorites_pdf_meaning"
    case settingsExportFavoritesPDFQuestion = "settings.export_favorites_pdf_question"
    case settingsExportFavoritesPDFAnswer = "settings.export_favorites_pdf_answer"
    case settingsExportFavoritesPDFExplanation = "settings.export_favorites_pdf_explanation"
    case settingsSupportSection = "settings.support_section"
    case settingsSupportHelpTitle = "settings.support_help_title"
    case settingsSupportHelpSubtitle = "settings.support_help_subtitle"
    case settingsSupportTermsTitle = "settings.support_terms_title"
    case settingsSupportTermsSubtitle = "settings.support_terms_subtitle"
    case settingsAboutRowTitle = "settings.about_row_title"
    case settingsAboutRowSubtitle = "settings.about_row_subtitle"
    case settingsSupportNavHelp = "settings.support_nav_help"
    case settingsSupportNavContact = "settings.support_nav_contact"
    case settingsSupportIntroTitle = "settings.support_intro_title"
    case settingsSupportIntroLine1 = "settings.support_intro_line1"
    case settingsSupportIntroLine2 = "settings.support_intro_line2"
    case settingsSupportIntroLine3 = "settings.support_intro_line3"
    case settingsSupportEmailLabel = "settings.support_email_label"
    case settingsSupportWebsiteLabel = "settings.support_website_label"
    case settingsSupportOr = "settings.support_or"
    case settingsSupportContactForm = "settings.support_contact_form"
    case settingsSupportAimRespond = "settings.support_aim_respond"
    case settingsSupportSectionFullName = "settings.support_section_full_name"
    case settingsSupportPlaceholderFullName = "settings.support_placeholder_full_name"
    case settingsSupportSectionYourEmail = "settings.support_section_your_email"
    case settingsSupportPlaceholderYourEmail = "settings.support_placeholder_your_email"
    case settingsSupportSectionSubject = "settings.support_section_subject"
    case settingsSupportSectionDescription = "settings.support_section_description"
    case settingsSupportAllRequired = "settings.support_all_required"
    case settingsSupportSend = "settings.support_send"
    case settingsSupportAlertSentTitle = "settings.support_alert_sent_title"
    case settingsSupportAlertSentMessage = "settings.support_alert_sent_message"
    case settingsSupportAlertMailFail = "settings.support_alert_mail_fail"
    case settingsTermsNavTitle = "settings.terms_nav_title"
    case settingsLegalTabTerms = "settings.legal_tab_terms"
    case settingsLegalTabPrivacy = "settings.legal_tab_privacy"
    case settingsLegalTabDisclaimer = "settings.legal_tab_disclaimer"
    case settingsAboutNavTitle = "settings.about_nav_title"
    case settingsAboutAppHeading = "settings.about_app_heading"
    case settingsAboutBody = "settings.about_body"
    case settingsAboutVersionFormat = "settings.about_version_format"

    // Common
    case commonSuccess = "common.success"
    case commonCancel = "common.cancel"
    case commonOK = "common.ok"
    case commonDone = "common.done"

    // Catalog errors
    case errorSignsMissing = "error.signs_missing"
    case errorSignsLoad = "error.signs_load"
    case errorTheoryMissing = "error.theory_missing"
    case errorTheoryLoad = "error.theory_load"

    // About
    case aboutSignImagesTitle = "about.sign_images.title"
    case aboutSignImagesBody = "about.sign_images.body"
    case aboutSource = "about.source"
    case aboutCommonsLink = "about.commons_link"
}
